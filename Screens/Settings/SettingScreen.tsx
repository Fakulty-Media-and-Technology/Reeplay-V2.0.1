...
import { getProfileDetails, setNotifications as notificationApi } from '@/api/profile.api';
...
const SettingScreen = () => {
  ...
  const url = 'https://reeplay-inc.gitbook.io/reeplay-inc.-docs/';
  ...
  const animatedStyle3 = useAnimatedStyle(() => {
    return {
      transform: [{translateX: animation3.value}],
    };
  });
  ...
  function handleAnimation3() {
    if (animation3.value === 0) {
      animation3.value = withTiming(11.5, {duration: 400});
    } else {
      animation3.value = withTiming(0, {duration: 400});
    }
  }
    ...
    <>
      <AppScreen
        ...
        {SettingTab.map((tabs, i) => {
          const odd = i % 2 === 0;
          const isInterest = tabs.toLowerCase().includes('interest');
          const isNotification = tabs.toLowerCase().includes('notifications');
          return (
            <TouchableOpacity
              key={i}
              onPress={() => handleClicked(tabs.toLowerCase())}
              activeOpacity={1}
              disabled={isInterest || isNotification}
              style={{
              ...
              <AppView className="flex-row items-center gap-x-4">
                ...
                <AppText className={`font-MANROPE_400 text-sm ${isInterest ?'text-[#626161]' :'text-white'} `}>
                  {tabs}
                </AppText>
              </AppView>
              ...
              <TouchableOpacity
                  className={`w-8 h-[19px] rounded-2xl pt-[1.5px] px-[2px] pb-[2.2px]  items-center flex-row ${
                    notifications ? 'bg-red' : 'bg-[#bbbbbb]'
                  }`}
                  activeOpacity={1}
                  onPress={() => [setNotifications(!notifications), handleAnimation3()]}>
                  <Animated.View style={[animatedStyle3, styles.circle]} />
                </TouchableOpacity>
              : (tabs.toLowerCase().includes('clear') || isInterest) ? (
                <></>
              ) : (
                <RightArrow />
              )}
