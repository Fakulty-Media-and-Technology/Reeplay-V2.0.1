...
import PaymentSummaryView from '../Menu/components/PaymentSummaryView';
import PayStackView from '../Menu/components/PayStackView';
...
const VoteScreen = () => {
  const {_id, voteInfo} = useRoute<VoteScreenRoute>().params;
  ...
  async function handleVoteInfo() {
    try {
      ...
      if (resContestants.ok && resContestants.data) {
        ...
        ToastNotification('error', `${resContestants.data?.message}`)
      }
    } catch (error) {
        ...
        <>
          {orientation === 'PORTRAIT' ? (
            <AppScreen containerStyle={{paddingTop: 15, paddingHorizontal: 0}}>
              ...
              <AppView className="w-full pl-5 mt-12 relative">
                {loading && <View className='w-full items-center mt-10 absolute z-20'><ActivityIndicator size={'large'} color={colors.RED} /></View>}
                <FlatList
                        ...
                        {contestantGroup.map((x, i) => {
                          ...
                          return (
                            <AnimatedView
                              ...
                              <AppImage
                                source={{uri: x.photo_url, priority:FastImage.priority.high}}
                                style={{
                                  width:58,
                                  height:58,
                                  overflow:'hidden',
    ...
    case 'payStack':
      return (
         <PayStackView
          ...
          saveCard={saveCard}
          voteInfo={{
            contestantId:activeId?activeId._id:'',
            voteInfoId:voteInfo._id
          }}
        />
      );
  }
